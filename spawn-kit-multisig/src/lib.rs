...
pub fn verify(params_json: &[u8]) -> Result<bool, ErrorCode> {
    ...
    let params: MultisigParams = serde_json_core::from_str::<MultisigParams>(params_str)
        .map(|(p, _)| p)
    ...
    if valid_count >= params.threshold && params.signatures.len() >= params.threshold as usize {
        Ok(true)
    } else {
        Ok(false)
    }
